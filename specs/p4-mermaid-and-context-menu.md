...
- FR-1：非源码模式识别语言名为 `mermaid` 的反引号或波浪线围栏代码块；块外光标显示图表，光标进入块内恢复开围栏、全部图表源码和闭围栏，并作为一个连续代码容器呈现以便编辑。
...
- AC-1.1：光标进入 Mermaid 块任意一行时，开闭围栏均可见且保持源码/显示索引一致；仅开围栏拥有顶部边界，仅闭围栏拥有底部边界，第一条图表语句不得再次生成顶部圆角或块间距。
